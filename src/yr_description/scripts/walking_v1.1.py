...
def make_x3(h):
    ...
    global v_des_robot,v_des_robot_feet,yaw_vel,leg_length_robot
    curr_euler_angls=[]#APR 5, 10.05 AM: get current euler angles
    rBody=euler_to_rotation(curr_euler_angls)
    #APR 5, 10.13 AM
    curr_angles=[]#current angles: [0,pel,hip,knee,ankle]
...
def solve_problem():
    #last_x: []
    #APR 5, 10.01 AM
    global h,keep_same,I_body,m,weights
    ...
    last_angles=[]#APR 5, 10.03 AM:CURRENT JOINT ANGLES: 0+angles
...
def get_torques(sides,last_angles):
    global needed_u
    last_angles=[]#READ ANGLES: [0,pelvis,hip,knee]
    torques={}
    if needed_u==None:
        return {}
...
def walk(torques,no_steps):
    ...
    fsm=[[i%2,(i+1)%2] for _ in range(no_steps)]
    for j in range(no_steps):#APR 5, 10.56 AM
      state_l,state_r=fsm[i]
      if state_l==0:
          lead,follow='r','l'
      else:
          lead,follow='l','r'
      last_angles=[]#READ ANGLES: [0,pelvis,hip,knee]x2
      torques=get_torques(lead)
      ...
      if lead=='l':
        final_pos=final_position(desired_velocity,curr_pos[0],r_body,hip_yaw_location,swing_time,1)
      elif lead=='r':
        final_pos=final_position(desired_velocity,curr_pos[1],r_body,hip_yaw_location,swing_time,1)
      saved_angles=[last_angles]
      saved_positions=[curr_pos]
      for i in range(no_steps_swing):
          phi=i/(no_steps_swing-1)
          new_pos=poly_curve(curr_pos,final_pos,phi)
          saved_positions.append(new_pos)
        #   last_angles=inverse_kinematics(new_pos)#INVERSE KINEMATICS: RETURN list
          if lead == 'l':
            last_angles = callback_manager.call_solve_ik_left_foot(new_pos[0],new_pos[1],new_pos[2],roll=0,pitch=0,yaw=0)
          else: 
            last_angles = callback_manager.call_solve_ik_right_foot(new_pos[0],new_pos[1],new_pos[2],roll=0,pitch=0,yaw=0)
